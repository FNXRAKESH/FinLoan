...
const Checkboxes = () => {
  ...
  useEffect(() => {
    if(localStorage.getItem('SP-personal-details-error')){
      setPd_color('#FD5E1A');
      setPd_icon(cross_icon);
    }

    if(localStorage.getItem('SP-currency-error')){
      setC_color('#FD5E1A');
      setC_icon(cross_icon);
    }

    if(localStorage.getItem('SP-terms-error')){
      setT_color('#FD5E1A');
      setT_icon(cross_icon);
    }


    if(localStorage.getItem('SP-settings-error')){
      setS_color('#FD5E1A');
      setS_icon(cross_icon);
    }

    if(localStorage.getItem('SP-MP-error')){
      setIrc_color('#FD5E1A');
      setIrc_icon(cross_icon);
    }

    if(localStorage.getItem('SP-charges-error')){
  ...
  return (
    <div 
      className="SP-Accounting-checkbox-div mx-auto d-flex my-4"
    >
      <div style={{width: '14%'}}>
      ...
      <div style={{width: '14%'}}>
        ...
        <h6 className="mt-3 text-center py-1" style={{color: `${irc_color}`}}>Market Price</h6>
      </div>
