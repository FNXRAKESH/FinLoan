...
const Checkboxes = () => {
  ...
  useEffect(() => {
    if(localStorage.getItem('LP-personal-details-error')){
      setPd_color('#FD5E1A');
      setPd_icon(cross_icon);
    }

    if(localStorage.getItem('LP-currency-error')){
      setC_color('#FD5E1A');
      setC_icon(cross_icon);
    }

    if(localStorage.getItem('LP-terms-error')){
      setT_color('#FD5E1A');
      setT_icon(cross_icon);
    }


    if(localStorage.getItem('LP-settings-error')){
      setS_color('#FD5E1A');
      setS_icon(cross_icon);
    }

    if(localStorage.getItem('LP-IRC-error')){
      setIrc_color('#FD5E1A');
      setIrc_icon(cross_icon);
    }

    if(localStorage.getItem('LP-charges-error')){
  ...
  return (
    <div 
      className="LP-Accounting-checkbox-div mx-auto d-flex my-4"
    >
      <div style={{width: '16%'}}>
